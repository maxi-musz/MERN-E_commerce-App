...
const CartScreen = () => {
  ...
  return (
    <Row>
      <Col md={8}>
        <h1>Shopping cart</h1>
        {cartItems.length === 0 ? <Message> Your cart is empty... <Link to='/'> Shop</Link> </Message> : (
          <ListGroup variant='flush'>
            {cartItems.map(item => (
              <ListGroup.Item key={item.product}>
                <Row>
                  <Col md={2}>
                    <Image className='product-description-image' src={item.image} alt={item.image} fluid rounded />
                  </Col>
                  <Col md={3}>
